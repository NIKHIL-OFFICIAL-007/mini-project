...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['cancel_order']) && isset($_POST['order_id'])) {
        ...
        if (empty($cancellation_reason)) {
            ...
            try {
                ...
                ");
                $history_stmt->execute([$order_id, $_SESSION['user_id'], "Cancelled by admin: " . $cancellation_reason]);
    ...
    if (isset($_POST['delete_order']) && isset($_POST['order_id'])) {
        ...
        try {
            ...
            $delete_history_stmt = $pdo->prepare("DELETE FROM order_status_history WHERE order_id = ?");
...
// Search filter
if ($search_term) {
    $sql_where[] = "(o.id LIKE ? OR u_buyer.name LIKE ? OR u_seller.name LIKE ? OR o.shipping_name LIKE ? OR o.shipping_email LIKE ?)";
...
<body class="bg-gray-50 text-gray-900">
    ...
    <div class="py-12 bg-gradient-to-r from-blue-600 to-blue-800 text-white">
        <div class="container mx-auto px-6 text-center">
            ...
            <p class="text-blue-100 max-w-2xl mx-auto text-lg">Manage and monitor all orders across the platform with full administrative control.</p>
        </div>
    </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-md">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">₹<?= formatPrice($total_revenue) ?></p>
                    </div>
            ...
            <div class="p-6">
                <form method="GET" action="manage_orders.php" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            <input type="text" name="search" placeholder="Search by order ID, buyer, seller, shipping name, or email..."
                            ...
                            <div class="border-t border-gray-100 pt-4 mt-4 space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    ...
                                    <a href="view_order.php?id=<?= $order['id'] ?>" 
                                       class="flex items-center justify-center px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition duration-150 text-sm font-semibold">
                                        <i class="fas fa-eye mr-1"></i> View
                                    </a>

                                    <!-- Delete Order -->
                                    <form method="POST" action="manage_orders.php" 
                                          onsubmit="return confirm('⚠️ WARNING: This will permanently delete Order #<?= $order['id'] ?> and all associated data. This action cannot be undone! Are you absolutely sure?');">
                                        <input type="hidden" name="order_id" value="<?= $order['id'] ?>">
                                        <button type="submit" name="delete_order" 
                                                class="w-full flex items-center justify-center px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition duration-150 text-sm font-semibold">
                                            <i class="fas fa-trash mr-1"></i> Delete
                                        </button>
                                    </form>
                                </div>

                                <!-- Cancel Order (if not already cancelled/delivered) -->
                                <?php if ($order['status'] !== 'cancelled' && $order['status'] !== 'delivered'): ?>
                                    <form method="POST" action="manage_orders.php" 
                                        ...
                                        <button type="submit" name="cancel_order" 
                                                class="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition duration-150 text-sm font-semibold">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const deleteButtons = document.querySelectorAll('button[name="delete_order"]');
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    if (!confirm('⚠️ WARNING: This will permanently delete the order and all associated data. This action cannot be undone! Are you absolutely sure?')) {
                        e.preventDefault();
                    }
                });
            });
