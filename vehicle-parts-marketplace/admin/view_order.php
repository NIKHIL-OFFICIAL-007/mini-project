...
if (!in_array('admin', $roles) || ($_SESSION['role_status'] ?? '') !== 'approved') {
    header("Location: ../login.php");
    exit();
}
        ...
        <div class="container mx-auto px-6">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-indigo-100 text-lg">Order #<?= $order['id'] ?> - <?= htmlspecialchars($order['buyer_name']) ?></p>
                </div>
                        ...
                        <?php if ($order['status'] !== 'cancelled' && $order['status'] !== 'delivered'): ?>
                            <form method="POST" action="view_order.php?id=<?= $order_id ?>" class="space-y-4">
                                ...
                                <button type="submit" name="update_status" 
                                        class="w-full px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg font-semibold">
