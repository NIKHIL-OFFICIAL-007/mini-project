...
// ----------------------------------------------------------------------
$per_page = 12; // Changed for grid layout
...
// ----------------------------------------------------------------------
// 3. HANDLE MODERATION ACTION (Hide/Show Review)
// ----------------------------------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['review_id'])) {
    ...
    try {
        ...
        if ($success) {
            $_SESSION['success'] = "Review ID {$review_id} has been set to '{$new_status}'.";
        } else {
        ...
        $_SESSION['error'] = "Database error: Could not process action.";
    }
    
    // Redirect back to the page with current filters
    $redirect_url = 'manage_reviews.php?' . http_build_query([
        'page' => $current_page,
...
if ($search_term) {
    $sql_where[] = "(p.name LIKE ? OR r.comment LIKE ? OR u_buyer.name LIKE ? OR u_seller.name LIKE ?)";
...
// 4.1 Get total count for pagination
$count_sql = "
    SELECT COUNT(r.id)
    FROM reviews r
...
// 4.2 Fetch reviews for the current page with additional data
$reviews = [];
$review_sql = "
    SELECT 
...
function get_pagination_url($page, $filter_rating, $filter_status, $search_term) {
    ...
    return 'manage_reviews.php?' . http_build_query(array_filter($query_params));
}
?>
...
<div class="py-12 bg-gradient-to-r from-blue-600 to-blue-800 text-white">
    <div class="container mx-auto px-6 text-center">
        ...
        <p class="text-blue-100 max-w-2xl mx-auto text-lg">Manage and moderate all customer reviews across the platform.</p>
    </div>
</div>
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-blue-100 overflow-hidden mb-8">
        ...
        <div class="p-6">
            <form method="GET" action="manage_reviews.php" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Search -->
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Search</label>
                    <div class="relative">
                        <input type="text" name="search" placeholder="Search by part, review, buyer, or seller..."
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <div class="flex items-center space-x-2">
                                <form method="POST" action="manage_reviews.php" 
                                      onsubmit="return confirm('Are you sure you want to <?= $review['status'] === 'active' ? 'HIDE' : 'SHOW' ?> this review?');"
                                      class="inline-block">
